...
const Sidebar = () => {
  ...
  return (
    <>
      {/* <div className="fs-3 fw-bold color-primary hamburger" onClick={handleHamburger()}>HA</div>
      <div className="fs-3 fw-bold text-primary cls" onClick={handleClose()}>CLS</div> */}
      {(config.layoutType === "dark-sidebar" || config.layoutType === "light-sidebar") && (
        <div ref={sidebarRef} id="kt_app_sidebar" className={clsx("app-sidebar bg-white", config.app?.sidebar?.default?.class)}>
